...
# Dice emojis that ACTUALLY animate in Telegram
DICE_EMOJIS = ["🎲", "🎯", "🏀", "⚽", "🎳", "🎰"]
PARTY_DICE = ["🎉"]  # Party popper dice

# Premium animated sticker placeholders
# In production, replace these with actual animated sticker file_ids
ANIMATED_STICKERS = {
    "welcome": [
        "CAACAgUAAxkBAAEBZJFkz...",  # Replace with real animated sticker file_id
    ],
    "party": [
        "CAACAgUAAxkBAAEBZJFkz...",
    ]
}
...
{deco} <b>Welcome to the Premium Experience</b> {deco}
</blockquote>
...
def get_random_dice():
    """Get a random animated dice emoji"""
    return random.choice(DICE_EMOJIS)

def get_party_dice():
    """Get party popper dice"""
    return "🎉"

def get_random_decorations(count=3):
    theme = get_theme(random_theme())
    return random.sample(theme["decorations"], min(count, len(theme["decorations"])))
