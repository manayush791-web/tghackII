...
Prevents 429 FloodWait errors with token-bucket & semaphore
"""
import asyncio
import time
...
class RateLimiter:
    """
    Per-bot rate limiter with:
    - Global semaphore (max concurrent API calls across ALL bots)
    - Per-chat rate limiting (20 msgs/min per chat)
    - Per-bot rate limiting (30 msgs/sec global)
    ...
    def __init__(self, max_concurrent=50, per_chat_limit=20, per_chat_window=60, per_bot_limit=30, per_bot_window=1):
        ...
        # Track timestamps per chat and per bot
