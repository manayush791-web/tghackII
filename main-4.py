...
Architecture: Flask handles ALL webhooks. Admin bot runs without its own server.
"""
import os
import sys
import json
...
from config import (
    ...
    AUTO_RESTART_ENABLED, AUTO_RESTART_INTERVAL_MIN
)
...
from themes import THEMES, DEFAULT_BUTTONS, format_welcome, truncate_text
from utils import parse_tokens, format_number, health, safe_api_call, hash_token
...
async def receive_tokens(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not tokens:
        await update.message.reply_text("❌ <b>No valid tokens found!</b>\nTry again.", parse_mode=ParseMode.HTML)
        return STATE_WAITING_TOKENS
    ...
    await update.message.reply_text(
        result_text,
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Panel", callback_data="admin_panel")]])
    )
    return STATE_IDLE
...
async def do_start_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_bot_running(token):
        await query.edit_message_text("⚠️ Bot is already running!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
        return STATE_IDLE
...
🚨 <code>{truncate_text(result, 100)}</code>"""
    ...
    await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Panel", callback_data="admin_panel")]]))
    return STATE_IDLE
...
async def stop_bot_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not running:
        await query.edit_message_text("❌ No bots are currently running.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
        return STATE_IDLE
    ...
    await query.edit_message_text("<b>🛑 Select a bot to STOP:</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(keyboard))
    return STATE_IDLE
...
async def do_delete_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("🗑️ <b>Bot deleted successfully!</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Panel", callback_data="admin_panel")]]))
    return STATE_IDLE
...
async def receive_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all_users:
        await message.reply_text("❌ No users found.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
        return STATE_IDLE
    ...
    async def status_callback(sent, failed, total):
        try:
            await status_msg.edit_text(
                f"⏳ <b>Broadcasting...</b>\nSent: {format_number(sent)} | Failed: {format_number(failed)}\nProgress: {sent+failed}/{format_number(total)}",
                parse_mode=ParseMode.HTML
            )
        except:
            pass
    ...
    await status_msg.edit_text(
        f"✅ <b>Broadcast Complete!</b>\n\n📤 Sent: {format_number(sent)}\n❌ Failed: {format_number(failed)}\n👥 Total: {format_number(len(all_users))}",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Panel", callback_data="admin_panel")]])
    )
...
async def receive_broadcast_per_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await update.message.reply_text(f"⏳ <b>Broadcasting to {format_number(len(users))} users...</b>", parse_mode=ParseMode.HTML)

    async def status_callback(sent, failed, total):
        try:
            await status_msg.edit_text(f"⏳ Broadcasting... {sent+failed}/{total}", parse_mode=ParseMode.HTML)
        except:
            pass
...
async def receive_channel_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    return STATE_IDLE
...
async def receive_welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ <b>Welcome message updated!</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Panel", callback_data="admin_panel")]]))
...
async def receive_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ <b>Buttons updated!</b> ({len(buttons)} buttons)", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    except Exception as e:
        await update.message.reply_text(f"❌ <b>Invalid JSON:</b> {e}\n\nTry again.", parse_mode=ParseMode.HTML)
        return STATE_WAITING_BUTTONS
...
async def receive_bio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ <b>Bio updated!</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
...
async def edit_theme_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard.append([InlineKeyboardButton("🔙 Back", callback_data="admin_panel")])
    await query.edit_message_text("<b>🎨 Choose a theme:</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def receive_block_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"🚫 <b>User {user_id} blocked.</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    except ValueError:
...
async def receive_unblock_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ <b>User {user_id} unblocked.</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    except ValueError:
        await update.message.reply_text("❌ Invalid user ID. Send a number.")
        return STATE_WAITING_CHANNEL
    return STATE_IDLE
...
async def export_tokens_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text("✅ <b>Export complete!</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    except Exception as e:
        await query.edit_message_text(f"❌ Export failed: {e}", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    return STATE_IDLE
...
async def backup_db_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text("✅ <b>Backup complete!</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    except Exception as e:
        await query.edit_message_text(f"❌ Backup failed: {e}", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    return STATE_IDLE
...
async def restart_all_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"✅ <b>Restart Complete!</b>\n\n🚀 Started: {started}\n❌ Failed: {failed}", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    return STATE_IDLE
...
async def stop_all_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"✅ <b>Stopped {stopped} bots</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]))
    return STATE_IDLE


# ─── Cancel / Fallback ───
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("❎ Cancelled.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Panel", callback_data="admin_panel")]]))
    return STATE_IDLE
...
async def fallback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        pass
    await update.message.reply_text("⚠️ Use the buttons or /start", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎛️ Open Panel", callback_data="admin_panel")]]))
    return STATE_IDLE
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            STATE_WAITING_TOKENS: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_tokens)],
            STATE_WAITING_BROADCAST: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_broadcast)],
            STATE_WAITING_BROADCAST_PER_BOT: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_broadcast_per_bot)],
            ...
            STATE_WAITING_CLONE_SOURCE: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_clone_source)],
            STATE_WAITING_CLONE_TARGET: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_clone_target)],
            STATE_WAITING_WELCOME: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_welcome)],
            STATE_WAITING_BUTTONS: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_buttons)],
            STATE_WAITING_BIO: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_bio)],
            STATE_WAITING_THEME: [CallbackQueryHandler(set_theme_callback, pattern="^set_theme:")],
        },
        fallbacks=[
    ...
    # Initialize and start the application (but don't start polling/webhook server)
    ...
    if WEBHOOK_URL:
        ...
        async def set_admin_webhook():
            ...
            print(f"✅ Admin webhook set: {WEBHOOK_URL}/webhook/admin")

        loop.run_until_complete(set_admin_webhook())

        # Start Flask webhook server in a background thread
        from webhook_server import setup_server, start_server
        setup_server(loop, app)

        flask_thread = threading.Thread(target=start_server, args=(HOST, PORT), daemon=True)
        flask_thread.start()

        # Keep main thread alive with the event loop
